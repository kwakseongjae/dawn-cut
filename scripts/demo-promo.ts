// 멀티 에셋 홍보영상 자동 생성 실증 (2026-06-11) —
// "상품 이미지 2 + 사용 영상 2(유/무음) + 자연어 한 문장 → 쇼츠형 홍보 영상"이
// 현재 프리미티브(EDL+오버레이+TTS+자막 번인+리프레임)로 끝까지 가능한지 증명한다.
//
// 파이프라인: LLM(홍보 대본 작성) → TTS '던'(보이스오버) → 무음 사용영상=베이스 →
// 유음 사용영상=B-roll 오버레이 → 상품 이미지 2장(pop 키프레임) → 배지/GIF 스티커 →
// 대본 자막 번인(쇼츠 프리셋) → vivid 색보정 → 9:16 리프레임 → mp4.
...
const OUT = join(ROOT, 'output/promo-demo');
...
const ASSETS = {
  productImages: [
    join(ROOT, 'output/sources/photo.jpg'), // 상품 이미지 1
    join(ROOT, 'output/responsive/01-small-top.png'), // 상품 이미지 2 (앱 화면)
  ],
  usageWithAudio: join(ROOT, 'output/sources/ko-talk.mp4'), // 사용 영상 1 (음성 있음 → B-roll)
  usageSilentSrc: join(ROOT, 'output/sources/clip.mp4'), // 사용 영상 2 (무음 처리 → 베이스)
};
const NL_REQUEST = '던컷 상품 홍보 영상 만들어줘 — 쇼츠 느낌으로, 핵심만 짧고 임팩트 있게';
...
제품: 던컷(dawn-cut) — 오픈소스 AI 영상 편집기. 100% 로컬 편집(영상이 기기를 안 떠남), 자동 한국어 자막, 무음 컷, 워터마크·구독료 없음.
...
JSON 배열로만 답해: ["문장1","문장2","문장3"]`,
  ...
  // B-roll — 유음 사용영상을 화면 하단 1/2 폭 윈도우로(중반).
  overlays.push({
    id: 'broll',
    kind: 'video',
    src: ASSETS.usageWithAudio,
    x: 0.04,
    y: 0.52,
    scale: 0.55,
    opacity: 1,
  ...
  const badgePng = join(OUT, 'badge.png');
  {
    ...
    drawBadge(c.getContext('2d') as unknown as DrawCtx, 420, 160, '100% 로컬 · 무료');
