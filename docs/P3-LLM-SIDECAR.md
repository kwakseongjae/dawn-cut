...
**정확도 레버 1 — few-shot (기본 적용, 라이선스 무관·footprint 0)**
`core.buildPlanPrompt`에 관찰된 1.5B 오답을 직접 겨냥한 few-shot 예시를 넣었다
(쨍→punch, 말버릇→removeFillers, 자막 스타일 객체 구조, 무의미 입력→`[]`). 실측상
"말버릇 정리" 류의 큰 오답이 교정됐다(단 "쨍"의 미세 오답·과잉생성은 잔존 — §5).

**정확도 레버 2 — 더 큰 모델 (env만 오버라이드, 코드 변경 0)**
`DAWN_LLM_MODEL_PATH`(+ setup의 `DAWN_LLM_MODEL_URL`)를 다른 GGUF로 지정하면 그대로
로드한다(인자·문법·ChatML 동일). **라이선스 주의**: 같은 패밀리라도 **Qwen2.5-3B는
Apache-2.0이 아니라 비상업 Qwen Research 라이선스**다. 오픈소스 dawn-cut의 기본 배포는
Apache-2.0인 1.5B를 유지하고, 더 큰 모델이 필요하면 **Qwen2.5-7B(Apache-2.0)** 가 안전한
선택이다(단 q4_k_m이 샤딩 GGUF라 다운로드가 무겁고 추론이 느림 → 상주 server 권장).
3B를 쓰려면 사용자가 라이선스를 직접 수용해야 한다.
...
- **지연**: CLI one-shot은 콜드 ~9s (모델 로드 ~7.7s + 평가 ~1.6s). 상주 server는 **웜
  ~0.1–0.5s**(실측: 두 번째 호출 67–100ms). timeout 기본 **120000ms**, 초과 시 kill +
  명확한 Error → 호출측(store)이 룰 경로로 폴백.

### 4.5 ★ 상주 `llama-server`가 기본 경로 (구현됨)
콜드 ~9s의 주범은 매 호출 모델 재로드다. 그래서 사이드카는 **상주 `llama-server`를 기본**으로
쓴다(`sidecar/llm/src/server.ts`). 모델을 1회만 로드해 두고 HTTP `/completion`으로 반복
추론 → 후속 호출이 평가시간 수준(~0.1–0.5s)으로 떨어진다.

- **lazy-spawn + 재사용**: 첫 요청에 `llama-server -m … -ngl 99 -c 4096 --host 127.0.0.1
  --port <DAWN_LLAMA_PORT|8127> --no-webui` 기동 → `GET /health`가 `{"status":"ok"}`가 될
  때까지 폴링 → 이후 동일 프로세스 재사용. 동시 호출은 단일 핸들의 `ready` 프라미스를 공유
  (이중 spawn 없음 — 핸들 할당이 첫 await 전에 동기적으로 끝난다).
- **워밍업**: store가 마운트 시 IPC `llm:warmup`을 fire-and-forget → 첫 사용자 요청 전에
  모델이 미리 로드돼 체감 콜드가 사라진다.
- **completion**: `POST /completion {prompt: ChatML, grammar: plannerGrammar(), n_predict,
  temperature, cache_prompt:true}` → `.content`. server `.content`는 보통 이미 깨끗하지만
  `cleanLlmOutput`을 동일 적용한다.
- **graceful fallback**: server 기동/추론 실패 시 `llmComplete`가 one-shot CLI로 폴백,
  CLI도 실패하면 throw → store가 룰 경로로. `DAWN_LLM_MODE=cli`면 처음부터 CLI만(결정성/CI).
  `DAWN_LLAMA_SERVER_URL`이 있으면 spawn 없이 그 외부 서버를 그대로 쓴다(테스트/파워유저).
- **정리(orphan 방지)**: Electron `app.on('will-quit')` → `shutdownLlm()`로 우리가 띄운
  서버만 종료. (사이드카도 `process.once('exit')` 동기 정리를 등록.)
...
| 변수 | 기본값 / 역할 |
|---|---|
| `DAWN_LLAMA_SERVER_BIN` | `vendor/llama.cpp/build/bin/llama-server` (상주, 기본 경로) |
| `DAWN_LLAMA_BIN` | `vendor/llama.cpp/build/bin/llama-cli` (one-shot 폴백) |
| `DAWN_LLM_MODEL_PATH` | `vendor/llama.cpp/models/qwen2.5-1.5b-instruct-q4_k_m.gguf` |
| `DAWN_LLM_MODE` | `server`(기본) \| `cli`(one-shot 강제 — CI/디버깅) |
| `DAWN_LLAMA_PORT` | `8127` (상주 서버 포트) |
| `DAWN_LLAMA_SERVER_URL` | (선택) 외부 관리 llama-server URL — 있으면 spawn 생략 |
| `DAWN_DISABLE_LLM` | (main) set이면 가용성 false 강제 → 룰 경로(e2e/CI 결정성) |
| `DAWN_LLM_MODEL_URL` | (setup만) 모델 다운로드 URL — 다른 GGUF로 오버라이드 |
| `DAWN_LLAMA_REPO` / `DAWN_LLAMA_REF` | (setup만) 빌드 리포/핀(`b4589`) |

`isLlmAvailable()`는 동기 `existsSync`로 model(존재 + 크기 > 100MB) + 바이너리(server 또는
cli 중 하나)를 확인하고(외부 URL이면 통과), 없으면 `available:false` + 한국어 `reason`을
**throw 없이** 돌려준다 → store가 안전히 룰 경로로 분기.

### `scripts/setup-llm.sh` (옵션·멱등)
1. llama.cpp 클론(핀 `b4589`) + cmake 빌드(Metal/Accelerate 자동) → `llama-cli` **+ `llama-server`**
2. Qwen2.5-1.5B-Instruct Q4_K_M GGUF 다운로드(~1.0GB) → models/
   (이미 있으면 건너뜀). 끝에 server/one-shot 검증 명령을 안내한다.
...
| `isLlmAvailable()` | 동기 존재 확인(model + server/cli 바이너리 OR 외부 URL). 없으면 `available:false` + 한국어 reason. throw 금지 |
| `LlmCompleteOpts` | `{ maxTokens?; timeoutMs?; grammar?; temperature? }` |
| `llmComplete(prompt, opts?)` | 기본 상주 server(`serverComplete`), 실패 시 one-shot CLI 폴백 → `{ text, ms }`. ChatML 래핑 + `plannerGrammar` |
| `warmupLlm()` | 상주 서버 미리 기동(앱 마운트 시) → `{ ready, ms, reason? }`. throw 금지 |
| `shutdownLlm()` | 우리가 띄운 상주 서버 종료(Electron `will-quit`에서 호출) |
| `isServerRunning()` | 상주 서버 가동 여부(외부 URL 포함) |
...
파일 구성: `chat.ts`(공유 헬퍼·env·ChatML·cleanLlmOutput), `server.ts`(상주 수명주기·HTTP),
`index.ts`(가용성·CLI 폴백·디스패치·공개 API).
...
가짜 `llama-cli`로 CLI 경로 + 가짜 HTTP `/completion`으로 server 경로 + core `planAndPreview`
end-to-end). 실모델 데모는 `tests/demo/llm-edit-demo.test.ts`(server 경로, `afterAll`로 정리).
...
3. 모델 — Qwen2.5-1.5B-Instruct Q4_K_M(~1.0GB, Apache-2.0); 정확도 레버=few-shot(기본)+
   더 큰 모델(7B Apache 권장, 3B는 비상업 라이선스 주의)
4. 검증된 실행 사실 — 정확한 인자, ChatML 래핑 필수, plannerGrammar 환각 봉쇄,
   ★상주 llama-server 기본(웜 ~0.1–0.5s) + warmup + CLI 폴백, few-shot
