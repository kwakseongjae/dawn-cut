...
## 2026-07-10 — B9 반응형 레이아웃 완료 (v0.1.19) — 사용자 피드백 즉시 대응
- 한 일: .main 컬럼(도크/자막 패널) clamp(vw) 유동화 + 타임라인 행 clamp(190px,24vh,340px)(전체화면=5레인 무스크롤) + 1120px 브레이크포인트(프리뷰 우선, keep-all) + 비디오 상한 calc 동기화. 2400/1000px 캡쳐 검증(output/b9-responsive/).
- 열린 것: 세밀 반응형(툴바 버튼 축약, 패널 접기)은 필요 시 후속.
- 다음: A2 (어시스턴트 채팅 패널)
