...
//
// 두 가지 문법을 노출한다:
//  - commandGrammar() : edit-command.ts의 13개 verb 전체(= MCP tool 표면과 1:1). 충실한 surface.
...
/**
 * EditCommand 13개 verb의 JSON 객체 배열을 표현하는 llama.cpp GBNF 문법 문자열을 반환한다.
 *
 * 13개 verb(deleteWordRange/removeSilences/removeFillers/cutSourceRange/applyGlossary/
 * setSubtitleStyle/replaceSubtitleStyle/highlightKeyword/applyColorgrade/applyZoom/
 * applyAutoEnhance/correctWord/autoHighlight)와 1:1. verb를 추가하면 cmd 대안과 규칙을 추가해야 한다.
 * (highlightKeyword·autoHighlight는 NL-안전이라 plannerGrammar에도 포함; applyAutoEnhance·correctWord는 외부전용이라 제외.)
 *
 ...
 * replaceSubtitleStyle, highlightKeyword, autoHighlight, applyColorgrade(preset, clipId 없음).
